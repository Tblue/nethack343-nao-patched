...
{\tt <space>} --- space key. \\
{\tt <esc>} --- escape key. \\
{\tt M-c} --- Meta c, where `c' is some character. \\
{\tt C-c} --- Control c, where `c' is some character. \\
{\tt \^{}c} --- same as ``C-c''. \\
{\tt nnn} --- three-digit decimal ASCII code, where `n' is 0-9.
%.ei
%.ed

For ``M-c'' and ``C-c'', the `M' and `C' are case-insensitive and the
