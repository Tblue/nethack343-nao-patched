...
%.lp
\item[\ib{directionkeys}]
Sets the directional keys to use. You can specify up to 10 keys; if you
specify less, these keys not specified are left at their defaults. The
first eight keys need to be lowercase letters; the last two keys can
also be these literal strings:
...
{\tt <space>} --- space key. \\
{\tt <esc>} --- escape key. \\
{\tt M-c} --- Meta {\tt c}, where `{\tt c}' is some character. \\
{\tt C-c} --- Control {\tt c}, where `{\tt c}' is some character. \\
{\tt \^{}c} --- same as ``{\tt C-c}''. \\
{\tt nnn} --- three-digit decimal ASCII code, where `{\tt n}' is 0-9.
%.ei
%.ed

For ``{\tt M-c}'' and ``{\tt C-c}'', the `{\tt M}' and `{\tt C}' are case-insensitive and the
hyphen is optional. For a clue in which order to specify the keys, see
the default value.
Default ``\verb&h y k u l n j b > <&''. Cannot be set with the `{\tt O}' command.
%.lp
\item[\ib{disclose}]
